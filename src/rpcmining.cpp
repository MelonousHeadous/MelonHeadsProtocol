...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "MelonHeadsProtocol is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "MelonHeadsProtocol is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "MelonHeadsProtocol is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "MelonHeadsProtocol is downloading blocks...");
